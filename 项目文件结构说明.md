...
- `mail-provider-utils.js`：网页邮箱 provider 配置纯工具，负责 `163 / 163 VIP / 126 / QQ / Yahoo / Inbucket / Hotmail / YYDS Mail` 的基础归一化与页面入口配置，并统一承接 iCloud 转发收码目标邮箱 provider 的归一化、选项列表和收码入口配置。
...
- `content/yahoo-mail.js`：Yahoo 邮箱轮询脚本，负责在 Yahoo Mail 页面中匹配验证码邮件，并在预览不足时回退到邮件正文提取验证码。
...
- `tests/yahoo-mail-content.test.js`：测试 Yahoo Mail 内容脚本对运行时验证码规则和轮询匹配路径的支持。
